    ...
    with open(args.csv_file, 'rt') as f:
        for line in f:
            ...
            if details.startswith('dzi'):
                examination_type, year = details.split('-')
                grade = 12
            else:
                examination_type, grade, year = details.split('-')

            print(get_airflow_trigger_command(examination_type, int(year), int(grade), resource_id, args.dry_run_value))
